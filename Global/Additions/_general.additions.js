...
.setColor("Red")
.setFooter({text: "Developed By luhux | Hata olduğunu düşünüyorsan geliştirici ile iletişime geç!", iconURL: message.guild.members.cache.get(client.owners[0]).user.avatarURL({dynamic:true})});
...
if(type == "memberYok") return message.reply({embeds:[hataEmbed.setDescription(`${hata} Bir kullanıcıyı etiketleyin **(\`@luhux\`)** veya **\`ID\`**'sini yazın.`)]}).then(async msg =>{
  setTimeout(async() => {
