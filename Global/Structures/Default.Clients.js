...
class luhux extends Client {
    constructor(options) {
        ...
        this.owners = options.owners || ["341592492224806914"]
    ...
    async connect(token = this.token) {
        ...
        if(this.MongoURI) {
            await mongoose.connect(this.MongoURI, {
                ...
                .then(async a => {
                this.guilds.cache.filter(guild => guild.id != Guild.ID).forEach(guild=> guild.leave().then(x=> console.log(`${guild.name}`)))
                }).catch(err => {
